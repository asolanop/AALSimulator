 ...
 * @author andressolano
 */
public class ShortSleep extends Event  {

    public ShortSleep (LocalTime start) {
    ...
    public void inform_agents(Agent agent) {
        ...
        agent.send(request);
        System.out.println("[SHORTSLEEP EVENT] Sendig event begin propagation to " + ((ElderAgent)agent).getHandlerAgent());
    }
    ...
    public void inform_agents_end(Agent agent) {
        ...
        request.setConversationId("aal.simulation.event");
        agent.send(request);
        System.out.println("[SHORTSLEEP EVENT] Sendig event end propagation to " + ((ElderAgent)agent).getHandlerAgent());
    }
    ...
    public int get_minutes_to_last() {
        ...
        int value = Math.abs(rand.nextInt() % 120) + 120;
        System.out.println("[EVENT SHORT SLEEP] Time to end: " + value);
